...
## 12. 열린 게시판 테스트 시나리오 및 결과

열린 게시판은 목격담/창작담을 분리해서 조회하고, 사용자가 직접 글을 작성한 뒤 목록, 상세, 추천, 수정, 삭제, 마이페이지 기록까지 이어지는 흐름을 기준으로 확인했다.

| 테스트 ID | 테스트 항목 | 시나리오 | 기대 결과 | 실제 결과 | 상태 |
|---|---|---|---|---|---|
| POST-01 | 목격담 목록 조회 | 열린 게시판 진입 후 `목격담` 탭 선택 | 목격담 게시글만 목록에 표시된다. | 목격담 탭에서 목격담 목록이 정상 표시됨 | 통과 |
| POST-02 | 창작담 목록 조회 | 열린 게시판 진입 후 `창작담` 탭 선택 | 창작담 게시글만 목록에 표시된다. | 창작담 탭에서 창작담 목록이 정상 표시됨 | 통과 |
| POST-03 | 게시글 검색 | 검색창에 키워드 입력 후 `SEARCH` 클릭 | 제목, 지역, 내용 기준으로 검색 결과가 필터링된다. | `옷장` 검색 시 관련 게시글만 목록에 표시됨 | 통과 |
| POST-04 | 검색 초기화 | 검색 결과 화면에서 `ALL` 클릭 | 검색 조건이 초기화되고 전체 목록이 다시 표시된다. | 전체 목록 화면으로 복귀됨 | 통과 |
| POST-05 | 목격담 작성 | `글쓰기` 클릭 후 제목, 지역, 내용을 입력하고 등록 | 새 목격담이 DB에 저장되고 목록 상단에 표시된다. | 작성한 목격담이 목록에 추가되고 조회 가능함 | 통과 |
| POST-06 | 게시글 상세 조회 | 목록에서 게시글 클릭 | 게시글 전체 내용, 지역, 조회수, 수정/삭제 버튼이 표시된다. | 상세 영역이 펼쳐지고 본문 전체가 표시됨 | 통과 |
| POST-07 | 게시글 추천 | 상세 화면에서 추천 버튼 클릭 | 추천 수가 1 증가하고 중복 추천은 제한된다. | 추천 수가 증가하고 DB의 추천 기록과 연결됨 | 통과 |
| POST-08 | 게시글 수정 | 본인 글 상세 화면에서 `수정` 클릭 후 내용 변경 | 수정 폼이 열리고 저장 시 변경 내용이 반영된다. | 수정 후 변경된 내용이 화면에 반영됨 | 통과 |
| POST-09 | 게시글 삭제 | 본인 글 상세 화면에서 `삭제` 클릭 | 게시글이 삭제되고 목록에서 사라진다. | 삭제 후 해당 게시글이 목록에서 제거됨 | 통과 |
| POST-10 | 마이페이지 작성글 확인 | 글 작성 후 `나의 보관함` 이동 | 사용자가 작성한 목격담/창작담 기록이 표시된다. | 마이페이지에서 작성한 목격담 기록 확인됨 | 통과 |

### 테스트 결과 요약

| 구분 | 결과 |
|---|---|
| 테스트 대상 | 열린 게시판 목격담/창작담 기능 |
| 주요 검증 범위 | 목록 조회, 탭 분리, 검색, 작성, 상세 조회, 추천, 수정, 삭제, 마이페이지 기록 |
| DB 연결 확인 | `post_post`, `post_like` 테이블을 통해 게시글과 추천 기록 저장 확인 |
| 최종 결과 | 주요 게시판 흐름 정상 동작 확인 |
